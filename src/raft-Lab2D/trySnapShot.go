...
// 	LeaderId int // 当前已知的leader ID，初始化为-1
...
// 	copyLogs := make([]LogEntries, endIdx-startIdx+1)

// 	// 2D 更新
// 	copy(copyLogs, rf.logs[startIdx-rf.lastIncludedIndex:endIdx-rf.lastIncludedIndex+1])
...
// 	for idx := len(rf.logs) - 1; idx >= 1; idx-- {
// 		logIndex := idx + rf.lastIncludedIndex
...
// 			if rf.role == Leader && rf.logs[idx].Term == rf.currentTerm {
// 				// 需要加上快照的最后一条索引
// 				rf.commitIndex = logIndex
// 				DPrintf("Leader %d updated commitIndex to %d", rf.me, logIndex)
// 			}
// 			break
// 		}
// 	}
...
// 		e.Encode(rf.lastIncludedIndex) != nil || e.Encode(rf.lastIncludedTerm) != nil {
...
// 	// 如果主动快照的index不大于rf之前的lastIncludedIndex，说明这次快照是重复或者更旧的，那么不应用此快照
...
// 	// 持久化存储
// 	rf.persist() // 持久化raft state
// 	state := rf.persister.ReadRaftState()
// 	rf.persister.SaveStateAndSnapshot(state, snapshot) // 持久化快照
...
// 	var newLog = []LogEntries{{Term: rf.logs[index-rf.lastIncludedIndex].Term, Index: index}} // 裁剪后依然log索引0处用一个占位entry，不实际使用
// 	newLog = append(newLog, rf.logs[index-rf.lastIncludedIndex+1:]...)                        // 这样可以避免原log底层数组由于有部分在被引用而无法将剪掉的部分GC（真正释放）
// 	rf.logs = newLog
...
// 				logIdx := nextId - rf.lastIncludedIndex
// 				if logIdx <= 0 || nextId >= len(rf.logs) {
// 					DPrintf("Server %d: 越界了，给节点 %d 发送的日志索引超出范围: nextId=%d, logs长度=%d, lastIncludedIndex=%d",
// 						rf.me, server, nextId, len(rf.logs), rf.lastIncludedIndex)
// 					continue
// 				}
...
// 				lastlLog := rf.logs[prevLogIdx] // 获取上次发送给follower的最后一条日志条目
...
// 				}(server, args)
// 			} else {
// 				// 修正：直接使用现有的 LeaderSendSnapshot 函数
// 				snapshotData := rf.persister.ReadSnapshot()
// 				go rf.LeaderSendSnapshot(server, snapshotData)
// 			}
...
// 					rf.nextIndex[id] = rf.logs[j].Index + 1
// 					flag = false
// 					break
...
// 	}
// 	// 安全访问：确保索引在有效范围内
// 	logIndex := args.PrevLogIdx - rf.lastIncludedIndex
...
// 			// 或者遍历到上个快照的最后一条日志的位置。
// 			for j := args.PrevLogIdx; j > rf.lastIncludedIndex; j-- {
// 				// 找到任期不匹配的条目时，记录其下一个条目的索引作为冲突索引，
// 				// 这意味着从此索引开始，日志条目需要重新同步
// 				if rf.logs[j-rf.lastIncludedIndex].Term != lastLog.Term {
...
// 		curIdx := idx + args.PrevLogIdx + 1
// 		realIdx := curIdx - rf.lastIncludedIndex // 在logs数组中的索引
// 		// 检查当前条目索引是否超出跟随者日志的长度，或者
// 		// 日志条目的任期号与跟随者现有日志条目中的任期号是否不一致
// 		if realIdx >= len(rf.logs) || rf.logs[curIdx].Term != args.Entries[idx].Term {
...
// 			rf.logs = append(rf.logs[:realIdx], args.Entries[idx:]...) // 此处修改了logs
// 			// 替换或追加完毕后，跳出循环
// 			break
// 		}
// 	}
...
// 		rf.commitIndex = min(rf.getLogLen()-1, args.LeaderCommit)
// 	}
...
// 	// leader向 server idx 发送InstallSnapshot RPC
// 	if ok := rf.sendSnapshot(idx, &args, &reply); !ok {
// 		return
// 	}
...
// 	// 处理RPC之前先判断是不是还是Leader
// 	if _, isLeader := rf.GetState(); !isLeader {
// 		return
// 	}
// 	// 防止任期混淆，当前收到旧任期的RPC回复，比较当前任期和原始RPC中发送的任期，两者不同则放弃回复并返回
// 	if rf.currentTerm != args.Term {
// 		return
// 	}
...
// // 被动快照
// // follower接受leader发来的InstallSnapshot RPC的handler
...
// 	// 接收leader的被动快照前先检查给server是否正在进行主动快照
...
// 	if isLeader { // 如果是leader收到AppendEntries RPC（虽然概率很小）
...
// 	// 说明snapshotIndex之前的log已经做成snapshot并删除了
...
// 	if snapshotIndex < rf.logs[len(rf.logs)-1].Index { // 若应用此次快照，本地还有日志要接上
...
// 	rf.persister.SaveStateAndSnapshot(rf.persister.ReadRaftState(), args.Data) // 持久化raft state及快照
...
// 		Snapshot:      args.Data,
// 	}
// 	rf.applyChan <- snapshotMsg // 将包含快照的ApplyMsg发送到applyCh，等待状态机处理
...
// // 简单的 HandleTimeout 实现
// func (rf *Raft) HandleTimeout() {
// 	// 由于你的 ticker 本身就是无限循环处理超时的
// 	// 这里只需要通过channel重置选举超时即可
// 	select {
// 	case rf.voteMsgChan <- struct{}{}:
// 	default:
// 	}
// }
...
// 		rf.applyChan <- msg // 将包含快照的ApplyMsg发送到applyCh，等待状态机处理
// 	}(snapshotMsg)
...
// 	rf.LeaderId = -1 // 初始化为-1表示无已知leader
