...
func TestInitialElection2A(t *testing.T) {
	...
	// is a leader elected? 检查是否选举出了一个领导者
	cfg.checkOneLeader()

	// sleep a bit to avoid racing with followers learning of the
	// election, then check that all peers agree on the term.
	// 睡眠一段时间以避免与追随者学习选举结果的竞争，然后检查所有节点是否同意任期。
	...
	// there should still be a leader. 仍然应该有一个领导者
	cfg.checkOneLeader()
...
func TestReElection2A(t *testing.T) {
	...
	// 如果旧的领导者重新连接，不应该影响新的领导者，旧的领导者应该切换到追随者
	cfg.connect(leader1)
...
func TestManyElections2A(t *testing.T) {
	...
	for ii := 1; ii < iters; ii++ {
		...
		// 记录当前状态
		//t.Logf("Iteration %d: disconnected nodes %d, %d, %d", ii, i1, i2, i3)
		...
		// 记录重新连接状态
		//t.Logf("Iteration %d: reconnected nodes %d, %d, %d", ii, i1, i2, i3)
	}
